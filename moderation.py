...
class Moderator:
    ...
    def check_duplicate_time_logic(self, deal_in, collection):
        ...
        days = deal_in['days_active']

        # Get the start and end times
        start_time = deal_in['start_time']
        end_time = deal_in['end_time']
        ...
        deals_f1 = [item for item in collection if set(item['days_active']).intersection(set(days))]
        ...
        deals_establishment_collection = [str(item['establishment']).lower().replace("'","").replace("bar","").replace("tavern","").replace("the","").replace("restaurant","").replace(" ","").strip() for item in deals_f1]
        estab_sim_list = self.check_duplication_by_embeddings(str(deal_in['establishment']).lower().replace("'","").replace("bar","").replace("tavern","").replace("the","").replace("restaurant","").replace(" ","").strip(), deals_establishment_collection)
